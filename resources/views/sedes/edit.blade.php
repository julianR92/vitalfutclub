                ...
                <section class="bg-white dark:bg-gray-900">
                    <div class="container px-6 py-7 mx-auto">
                        <div class="bg-slate-100/50 py-1 px-3 border-solid border-b rounded-lg mb-4">
                            <x-titulo titulo="Editar Sede"></x-titulo>
                        </div>

                        <form action="{{ route('sedes.update', $sede) }}" method="POST" class="w-full max-w-full p-2">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="w-full md:w-1/4 px-1 mb-2 md:mb-0">
                                    ...
                                    <select name="ciudad_id"
                                        ...
                                        @foreach($ciudades as $ciudad)
                                            <option value="{{ $ciudad->id }}" {{ old('ciudad_id', $sede->ciudad_id) == $ciudad->id ? 'selected' : '' }}>
                                                {{ $ciudad->nombre }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="w-full md:w-1/4 px-1 mb-2 md:mb-0">
                                    ...
                                    <select name="user_id"
                                        ...
                                        @foreach($profesores as $profesor)
                                            <option value="{{ $profesor->id }}" {{ old('user_id', $sede->user_id) == $profesor->id ? 'selected' : '' }}>
                                                {{ $profesor->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="flex flex-wrap mt-3">
                                <div class="w-full md:w-full px-1 mb-2 md:mb-0">
                                    ...
                                    <input type="text"
                                           name="persona_cargo"
                                           id="persona_cargo"
                                           value="{{ old('persona_cargo', $sede->persona_cargo) }}"
